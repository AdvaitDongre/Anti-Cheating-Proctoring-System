...
class GeminiSpeakerDetector:
    def __init__(self, api_key, format=pyaudio.paInt16, channels=1, rate=16000, chunk=1024, 
                 record_seconds=3, reference_audio=None):
        self.format = format
    ...
    def _compare_with_reference(self):
        """Compare recorded audio with reference audio"""
        try:
            # Basic audio comparison (spectral analysis)
            ref_rate, ref_data = wavfile.read(self.reference_audio)
            current_rate, current_data = wavfile.read(self.temp_wav)
            
            # Resample if necessary
            if ref_rate != current_rate:
                num_samples = int(len(current_data) * float(ref_rate) / current_rate)
                current_data = signal.resample(current_data, num_samples)
            
            # Simple correlation check
            correlation = np.correlate(ref_data[:len(current_data)], current_data, mode='valid')[0]
            threshold = 0.7  # Adjust based on your needs
            
            if correlation < threshold:
                print("\033[91m[ALERT] AUDIO MISMATCH DETECTED!\033[0m")
            else:
                # If audio matches, still check for speaker count
                self._analyze_audio()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--reference', type=str, help='Path to reference audio file for comparison', default=None)
    args = parser.parse_args()
    
    # Initialize and start the detector
    detector = GeminiSpeakerDetector(
        api_key=API_KEY, 
        record_seconds=3,
